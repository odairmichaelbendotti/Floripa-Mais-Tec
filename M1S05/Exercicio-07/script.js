document.addEventListener("DOMContentLoaded", function() {
    ...
    function updateSaleDetails() {
        ...
        localStorage.setItem("orderList", orderList.innerHTML);
    ...
    // Função para simular a compra e imprimir recibo
    buyButton.addEventListener("click", function() {
        const totalAmount = totalAmountDisplay.textContent;
        alert(`Compra realizada com sucesso!\nValor da compra: ${totalAmount}\nO recibo será impresso.`);
    });

    // Ler os dados do localStorage ao iniciar a aplicação
    const savedOrderList = localStorage.getItem("orderList");
    if (savedOrderList) {
        orderList.innerHTML = savedOrderList;
        totalAmountDisplay.textContent = `R$ ${localStorage.getItem("totalAmount")}`;
        totalProductsDisplay.textContent = localStorage.getItem("totalProducts");
    }
});
